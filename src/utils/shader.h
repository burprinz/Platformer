...
class Shader {
	public:
		...
		inline const Shader& setVec2(const char* name, const glm::vec2 vector) const noexcept {
			glUniform2fv(glGetUniformLocation(m_handle, name), 1, glm::value_ptr(vector));
			return *this;
		}
		...
		inline const Shader& setVec3_f(const char* name, const glm::vec3 vector) const noexcept {
			glUniform3f(glGetUniformLocation(m_handle, name), vector.x, vector.y, vector.z);
			return *this;
		}
		inline const Shader& setVec4(const char* name, const glm::vec4 vector) const noexcept {
			glUniform4fv(glGetUniformLocation(m_handle, name), 1, glm::value_ptr(vector));
			return *this;
		}
